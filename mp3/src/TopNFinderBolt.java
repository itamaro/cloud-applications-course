...
public class TopNFinderBolt extends BaseBasicBolt {
  ...
  public void execute(Tuple tuple, BasicOutputCollector collector) {
    ...
    while (currentTopWords.size() > this.N) {
      // trim the map
      Integer min_count = count;
      String word_to_trim = word;
      for (String other_word : currentTopWords.keySet()) {
        Integer other_count = currentTopWords.get(other_word);
        if (other_count < min_count) {
          min_count = other_count;
          word_to_trim = other_word;
        }
      }
      currentTopWords.remove(word_to_trim);
    }
